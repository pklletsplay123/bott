function help() {
    return `
🔰 -----[ *IZUMI-BOT* ]----- 🔰

Berikut adalah beberapa command yang ada pada bot ini!✨

♻ Command Group Only :
  
  ⚜ *!groupinfo* 🆕
  ⚜ *!add 62858xxxxx*
  ⚜ *!kick @tagmember*
  ⚜ *!promote @tagmember*
  ⚜ *!demote @tagadmin*
  ⚜ *!mentionAll*
  ⚜ *!adminList*
  ⚜ *!ownerGroup*
  ⚜ *!leave*
  ⚜ *!linkGroup*
  ⚜ *!delete (replyChatBot)*
  ⚜ *!kickAll*
  ⚜ *!NSFW [enable|disable]*
  ⚜ *!welcome [enable|disable]*

♻ Command Downloader :

  ⚜ *~!ytmp3 [linkYt]~* 🚫
  ⚜ *~!ytmp4 [linkYt]~* 🚫
  ⚜ *~!ig [linkIg]~* 🚫
  ⚜ *~!fb [linkFb]~* 🚫
  ~Command *Downloader* jangan digunakan terlebih dahulu 🙏🏼

♻ Command Random *IZUMI-BOT* :

   ⚜ *!info*
   ⚜ *!snk*
   ⚜ *!owner*
   ⚜ *!donasi*
   ⚜ *!infosurah* 🆕
   ⚜ *!randomsurah* 🆕
   ⚜ *!manga [Judul Manga]* 🆕
   ⚜ *!corona* 🆕
   ⚜ *!url2img* 🆕
   ⚜ *!sticker*
   ⚜ *!stickerGif*
   ⚜ *!stickernobg*
   ⚜ *!neko*
   ⚜ *!pokemon*
   ⚜ *!inu*
   ⚜ *!ss*
   ⚜ *!jadwalShalat [daerah]*
   ⚜ *!jadwalTv [channel]*
   ⚜ *!jadwaltvnow [channel]* 🆕
   ⚜ *!cuaca [tempat]*
   ⚜ *!tts [kode bhs] [teks]*
   ⚜ *!igStalk [@username]*
   ⚜ *!nh [kode nuklir]*
   ⚜ *!wiki [query]*
   ⚜ *!anime [Judul Anime]*
   ⚜ *!brainly [pertanyaan] [.jumlah]*
   ⚜ *!loli*
   ⚜ *!waifu*
   ⚜ *!husbu*
   ⚜ *!randomNekoNime* 🚫
   ⚜ *~!randomTrapNime~* 🚫
   ⚜ *~!randomAnime~* 🚫
   ⚜ *!infoGempa*
   ⚜ *!meme*
   ⚜ *!quotemaker [|teks|author|theme]*
   ⚜ *!join [linkGroup]*
   ⚜ *!quotes*
   ⚜ *!quotesnime*
   ⚜ *!wait*
   ⚜ *!nulis [teks]*
   ⚜ *!lirik [optional]*
   ⚜ *!chord [query]*
 👉🏼Kirim perintah *!readme* untuk mengetahui fungsi dan cara penggunaan perintah di atas, WAJIB BACA ‼️.

 ♻ Owner BOT Only :

   ⚜ *!bc [boardcast]*
   ⚜ *!getses [screenshot whatsapp interface]*
   ⚜ *!leaveall [keluar dari semua group]*
   ⚜ *!clearall [membersihkan isi chat]*
   ⚜ *!listblock [Melihat daftar user banned]*
   ⚜ *!sendto*


⚠️ Kirim perintah *!snk* untuk membaca syarat & ketentuan yang berlaku
⚠️ Jangan melakukan spam command ‼️
⚠️ Telfon atau VC BOT langsung banned ‼️
⚠️ Gunakan bot dengan bijak ‼️
⚠️ Bot ini online 24 Jam selama tak terjadi error ‼️
  
 -----[ *POWERED BY IZUMI-BOT* ]----- `
}
exports.help = help()
function readme() {
    return `
*[ss]* Diisi dengan link website yang valid tanpa tanda “[” dan “]”
Contoh : *!ss https://www.nekopoi.care*

*[linkYt]* Diisi dengan link YouTube yang valid tanpa tanda “[” dan “]”
Contoh : *!ytmp3 https://youtu.be/DMAmaISCLag*

*[linkYt]* Diisi dengan link YouTube yang valid tanpa tanda “[” dan “]”
Contoh : *!ytmp4 https://youtu.be/DMAmaISCLag*

*[linkIg]* Diisi dengan link Instagram yang valid tanpa tanda “[” dan “]”
Contoh : *!ig https://www.instagram.com/p/CFqRZTlluAi/?igshid=1gtxkbdqhnbbe*

*[linkFb]* Diisi dengan link Facebook yang valid tanpa tanda “[” dan “]”
Contoh : *!fb https://www.facebook.com/EpochTimesTrending/videos/310155606660409*

*[daerah]* Diisi dengan daerah yang valid, tanpa tanda “[” dan “]”
Contoh : *!jadwalShalat trenggalek*

*[channel]* Diisi dengan channel televisi yang valid, tanpa tanda “[” dan “]”
Contoh : *!jadwalTv Indosiar*

*[tempat]* Diisi dengan tempat/lokasi yang valid, tanpa tanda “[” dan “]“
Contoh : *!cuaca trenggalek*

*[kode bhs]* Diisi dengan kode bahasa, contoh *id*, *en*, dll. Dan *[teks]* Diisi dengan teks yang ingin di jadikan voice, Masih sama seperti di atas tanpa tanda “[” dan “]”
Contoh : *!tts id Test*
Note : Max 250 huruf

*[@username]* Diisi dengan username Instagram yang valid, tanpa tanda “[” dan “]”
Contoh : *!igStalk @duar_amjay*

*[|teks|author|theme]* Diisi dengan teks, author, dan theme, tanpa tanda “[” dan “]”
Contoh : *!quotemaker |santet anak DPR|IZUMI-BOT*

*[linkGroup]* Diisi dengan link group whatsapp yang valid, tanpa tanda “[” dan “]”.
Contoh : *!join https://chat.whatsapp.com/EnalEgnJt5qH9LGncdZyx4*

*[optional]* Diisi dengan teks|title lirik lagu, tanpa tanda “[” dan “]”.
Contoh : *!lirik ref:rain*`
}
exports.readme = readme()
function info() {
    return `Bot ini merupakan ai responder berbasis nodeJS dan JS
Owner Bot : https://wa.me/6287743210434
Author? : Ada pokoknya om, malas pasang nama doang`
}
exports.info = info()
function snk() {
    return `Syarat dan Ketentuan *IZUMI-BOT*
1. Teks dan nama pengguna WhatsApp anda akan kami simpan di dalam server selama bot aktif
2. Data anda akan di hapus ketika bot Offline
3. Kami tidak menyimpan gambar, video, file, audio, dan dokumen yang anda kirim
4. Kami tidak akan pernah meminta anda untuk memberikan informasi pribadi
5. Jika menemukan Bug/Error silahkan langsung lapor ke Owner bot (https://wa.me/6287743210434)
6. Apapun yang anda perintah pada bot ini, KAMI TIDAK AKAN BERTANGGUNG JAWAB!
7. Jangan sekali kali mencoba untuk menelpon BOT jika tak ingin di banned oleh BOT
8. Untuk donasi bisa melalui command *!donasi* atau melalui https://saweria.co/dhanifitrah5

Thanks !`
}
exports.snk = snk()
function donate() {
    return `Ya halo om.. Mau donate?
    
Kalo mau donate langsung ae ke :
gas ae chat admin bot untuk info selengkapnya (https://wa.me/6287743210434)

untuk donasi bisa dikirim melalui via dibawah ini :

❥via Pulsa : 087833474586 atau 081803441134
❥via DANA : https://link.dana.id/qr/2obla0p2
❥via LinkAja : 087833474586
❥via GoPay : -
❥via OVO : -

Thanks !`
}
exports.donate = donate()
function listChannel() {
    return `Daftar channel: 
1. ANTV
2. GTV
3. Indosiar
4. iNewsTV
5. KompasTV
6. MNCTV
7. METROTV
8. NETTV
9. RCTI
10. SCTV
11. RTV
12. Trans7
13. TransTV`
}
exports.listChannel = listChannel()
function ApiKey() {
    return `lGjYt4zA5SQlTDx9z9Ca`
}
exports.ApiKey = ApiKey()
